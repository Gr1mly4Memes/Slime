...
public enum Material implements Keyed, Translatable, net.kyori.adventure.translation.Translatable { // Paper
    ...
    // Slime start
    private static Material addEnumValue(String materialName, int id, boolean isBlock, boolean isItem) {
        try {
            Field valuesField = Material.class.getDeclaredField("$VALUES");
            valuesField.setAccessible(true);
            @SuppressWarnings("unchecked")
            Material[] oldValues = (Material[]) valuesField.get(null);
            Material[] newValues = new Material[oldValues.length + 1];
            System.arraycopy(oldValues, 0, newValues, 0, oldValues.length);

            Constructor<Material> constructor = Material.class.getDeclaredConstructor(int.class, boolean.class, boolean.class);
            constructor.setAccessible(true);
            Material newMaterial = constructor.newInstance(id, isBlock, isItem);
            
            Field nameField = Enum.class.getDeclaredField("name");
            nameField.setAccessible(true);
            nameField.set(newMaterial, materialName);
            
            Field ordinalField = Enum.class.getDeclaredField("ordinal");
            ordinalField.setAccessible(true);
            ordinalField.setInt(newMaterial, oldValues.length);
            
            newValues[oldValues.length] = newMaterial;
            valuesField.set(null, newValues);
            return newMaterial;
        } catch (Exception e) {
            throw new RuntimeException("Failed to add enum value: " + materialName, e);
        }
    }
    ...
    public static Material addMaterial(String materialName, int id, boolean isBlock, boolean isItem, Identifier resourceLocation) {
        if (isBlock) {
            ...
            if (material != null) {
                ...
                material = addEnumValue(materialName, id, isBlock, isItem);
            }
            ...
            Material material = addEnumValue(materialName, id, isBlock, isItem);
            BY_NAME.put(materialName, material);
            material.key = CraftNamespacedKey.fromMinecraft(resourceLocation);
